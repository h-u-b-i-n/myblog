...
public class IndexController {
    ...
    @PostMapping("/search")         //提及在表单里 用的post
    public String search(@PageableDefault(size=8,sort={"updateTime"},direction = Sort.Direction.DESC)
                                 Pageable pageable,@RequestParam String query, Model model){  //query是搜索内容（在_fragments中）
        model.addAttribute("page",blogService.listBlog("%"+query+"%",pageable));
        model.addAttribute("query",query);   //搜索框里显示的还是输入的query内容
        return "search";
    }
